...
async function startRecursive(users = [], sum = 0) {
 if (users.length) {
  const usersSlice = users.splice(0,10);
  console.log(usersSlice);
  const usersSliceAsync = await Promise.all(usersSlice.map(async user => processUser(user.id)));
  
  let idSum = usersSliceAsync.reduce((sum, id) => sum + id);
  idSum += sum;
  return startRecursive(users, idSum);
